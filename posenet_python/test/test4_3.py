...
def squat_down(ratio, ratio_arr, grad, squat_knee_angle, left_knee_angle, right_knee_angle, left_hip_gap, right_hip_gap,  standing_ratio):
    ...
    if grad <= -2: # 내려갈 때
        ...
        if (squat_knee_angle <= left_knee_angle < squat_knee_angle * 1.2 or 
                ...
                if count_flag == False:
                    winsound.Beep(491, 250)

                count_flag = True
                color = (255,0,0)

        if ratio > ratio_arr[-2]: # 올라갈 때
            # print(grad)
            # print('올라갈 때 ratio:', ratio)
            test = ""
            color = (0,0,250)

            if abs(grad) < 0.1 and count_flag:
                count_flag = False
                squat_count += 1
                winsound.Beep(291, 250)
                print('다시 올라옴',squat_count)
        ...
        with tf.Session() as sess:
            ...
            while self.running:
                ...
                if ret:
                    ...
                    if real_start:
                        grad = (sum(ratio_arr[-10:-5]) - sum(ratio_arr[-5:-1]))/10
